...
int sm4_ecb_padding_encrypt(const SM4_KEY *key, const uint8_t *in, size_t inlen, uint8_t *out, size_t *outlen)
{
	uint8_t block[16];
	size_t rem = inlen % 16;
	int padding = 16 - inlen % 16;
	...
	memset(block + rem, padding, padding);
	if (inlen/16) {
		sm4_encrypt_blocks(key, in, inlen/16, out);
		out += inlen - rem;
	}
	sm4_encrypt_blocks(key, block, 1, out);
	*outlen = inlen - rem + 16;
	return 1;
}

int sm4_ecb_padding_decrypt(const SM4_KEY *key, const uint8_t *in, size_t inlen, uint8_t *out, size_t *outlen)
{
	uint8_t block[16];
	size_t len = sizeof(block);
	int padding;
	...
	if (inlen > 16) {
		sm4_encrypt_blocks(key, in, inlen/16 - 1, out);
	}
	sm4_encrypt_blocks(key, in + inlen - 16, 1, block);
...
int sm4_ecb_encrypt_update(SM4_ECB_CTX *ctx, const uint8_t *in, size_t inlen, uint8_t *out, size_t *outlen)
{
	size_t left;
	size_t nblocks;
	size_t len;
...
int sm4_ecb_decrypt_update(SM4_ECB_CTX *ctx, const uint8_t *in, size_t inlen, uint8_t *out, size_t *outlen)
{
