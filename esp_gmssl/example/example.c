...
int test_sm4_ecb_padding(void)
{
	uint8_t user_key[16] = "1234567890123456";
	SM4_KEY sm4_key = { 0 };
	sm4_set_encrypt_key(&sm4_key, user_key);

	uint8_t in[] = "123456abcd12312312312aaaa";
	size_t inlen = strlen((char*)in);
	uint8_t out[inlen];
	size_t outlen = 0;
	sm4_ecb_padding_encrypt(&sm4_key, in, inlen, out, &outlen);
	ESP_LOG_BUFFER_HEX("ecb encrypt", out, outlen);

	SM4_KEY sm4_key2 = { 0 };
	uint8_t out2[inlen];
	size_t outlen2 = 0;
	sm4_set_decrypt_key(&sm4_key2, user_key);
	sm4_ecb_padding_decrypt(&sm4_key2, out, outlen, out2, &outlen2);
	// ESP_LOG_BUFFER_HEX("ecb decrypt", out2, outlen2);
	ESP_LOGI("ecb decrypt", "%.*s", (int)outlen2, out2);
