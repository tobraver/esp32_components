...
char* get_network_mask(){
    return network_mask;
}

char* get_network_dns(){
    return network_dns;
}
...
static esp_err_t print_all_ips_tcpip(void* ctx)
{
    ...
    while ((netif = esp_netif_next_unsafe(netif)) != NULL) {
        if (example_is_our_netif(prefix, netif)) {
            ...
            ESP_LOGI(TAG, "- IPv4 mask: " IPSTR, IP2STR(&ip.netmask));
            ESP_LOGI(TAG, "- IPv4 gateway: " IPSTR, IP2STR(&ip.gw));

            esp_netif_dns_info_t dns_info;
            esp_netif_get_dns_info(netif, 0, &dns_info);
            ESP_LOGI(TAG, "- IPv4 dns: " IPSTR, IP2STR(&dns_info.ip.u_addr.ip4));

            sprintf(network_ip,IPSTR, IP2STR(&ip.ip));
            sprintf(network_mask,IPSTR, IP2STR(&ip.netmask));
            sprintf(network_gateway,IPSTR, IP2STR(&ip.gw));
            sprintf(network_dns,IPSTR, IP2STR(&dns_info.ip.u_addr.ip4));
#endif
#if CONFIG_EXAMPLE_CONNECT_IPV6
